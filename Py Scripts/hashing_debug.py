...
def main():
    ...
    # Output the category distribution
    print("Category Distribution:")
    print(libri2mix_overall_clean_detailed["type"].value_counts())

    # Check the hash distribution before splitting
    libri2mix_overall_clean_detailed["hashkey"] = (
        libri2mix_overall_clean_detailed["audio_id"] + libri2mix_overall_clean_detailed["source1_ranges_list"]
    ).apply(hash)
    libri2mix_overall_clean_detailed["hash_%10"] = libri2mix_overall_clean_detailed["hashkey"] % 10
    print("Hash Distribution:")
    print(libri2mix_overall_clean_detailed["hash_%10"].value_counts())

    # Save the combined DataFrame
    libri2mix_overall_clean_detailed.to_csv("/home/users/ntu/scsekyad/scratch/OSD/final deployment package/klass-osd-kedro-pipeline/klass-osd/data/02_intermediate/hash_libri2_detailed/libri2mix_clean_detailed_datasets_final_x.csv", index=False)
    ...
    libri2mix_modeling_dataset.to_csv("/home/users/ntu/scsekyad/scratch/OSD/final deployment package/klass-osd-kedro-pipeline/klass-osd/data/02_intermediate/hash_libri2_detailed/libri2mix_modeling_dataset_final_x.csv", index=False)

    # Output the count of entries in the modeling dataset
    print("Modeling Dataset Entry Count:")
    print(libri2mix_modeling_dataset["split"].value_counts())
